        ...
        stages {
          ...
          stage('Build') {
            ...
            steps {
              ...
              dir('tmp') {
                timestamps {
                  sh '../scripts/firedrake-install $COMPLEX --tinyasm --package-branch tsfc gpu --package-branch PyOP2 gpu --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install irksome --no-package-manager || (cat firedrake-install.log && /bin/false)'
                }
              }
            }
          }
