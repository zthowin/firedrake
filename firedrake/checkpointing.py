...
from firedrake.functionspacedata import get_global_numbering, preprocess_ufl_element
...
class DumbCheckpoint(object):
    ...
    .. warning::
       ...
       DumbCheckpoint class will be deprecated after 01/01/2022.
    ...
    def __init__(self, basename, single_file=True,
        ...
        with warnings.catch_warnings():
            ...
            warnings.warn("DumbCheckpoint class will be deprecated after 01/01/2022; use CheckpointFile class instead.",
                          DeprecationWarning)
...
class HDF5File(object):
    ...
    .. warning::
       ...
       HDF5File class will be deprecated after 01/01/2022.
       Use :class:`~.CheckpointFile` class instead.
    ...
    def __init__(self, filename, file_mode, comm=None):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter('always', DeprecationWarning)
            warnings.warn("HDF5File class will be deprecated after 01/01/2022; use CheckpointFile class instead.",
                          DeprecationWarning)
        self.comm = dup_comm(comm or COMM_WORLD)
...
class CheckpointFile(object):
    ...
    def _generate_function_space_name(self, V):
        ...
        for Vsub in V:
            V_names.append("_".join([Vsub.mesh().name, str(Vsub.ufl_element())]))
    ...
    def _get_shared_data_key_for_checkpointing(self, mesh, ufl_element):
        _, _, nodes_per_entity, real_tensorproduct = preprocess_ufl_element(mesh, ufl_element)
